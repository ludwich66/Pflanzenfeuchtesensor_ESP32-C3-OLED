"""
ESP32-C3-OLED Kapazitiver Feuchtesensor - Version 3.1a FINAL 29-10-2025
============================================================
...
# ====================================================================
# SENSOR-KALIBRIERUNG UND ICON-ZUORDNUNG - Version 3.1a FINAL
# ZENTRALE KONFIGURATION - Alle Werte an einem Ort!
# ====================================================================

# ADC-Bereiche und Icon-Zuordnung (von trocken nach nass, 84er Schritte)
# 5 eindeutige MQTT-Status für bessere Differenzierung!
MOISTURE_ZONES = [
    # (Name, ADC_Min, ADC_Max, Icon_ID, Status_Text, MQTT_Status)
    ("SEHR_TROCKEN", 767,  850,  5, " GIESSEN!", "SDRY"),   # V: Super Dry
    ("TROCKEN",      684,  766,  4, " WELKEND",  "DRY"),    # IV: Dry
    ("OPTIMAL",      587,  683,  3, " OPTIMAL",  "OK"),     # III: OK
    ("FEUCHT",       504,  586,  2, " WACHSEND", "WET"),    # II: Wet
    ("SEHR_FEUCHT",  420,  503,  1, "  FEUCHT",  "SWET"),   # I: Super Wet
]
...
# ====================================================================
# 5-STUFEN DISPLAY-SYMBOLE (40x40 Pixel) - Pixil-optimiert
# ====================================================================
...
def get_moisture_zone(adc_value):
    ...
    Returns: tuple (zone_name, icon_id, status_text, mqtt_status)
    """
    for name, adc_min, adc_max, icon_id, status_text, mqtt_status in MOISTURE_ZONES:
        if adc_min <= adc_value <= adc_max:
            return (name, icon_id, status_text, mqtt_status)

    # Fallback
    if adc_value > 850:
        return ("SEHR_TROCKEN", 5, " GIESSEN!", "SDRY")
    else:
        return ("SEHR_FEUCHT", 1, "  FEUCHT", "SWET")

def determine_moisture_status(adc_value):
    """
    Bestimmt den Feuchtigkeitsstatus basierend auf MOISTURE_ZONES

    Returns: "SWET", "WET", "OK", "DRY", oder "SDRY" (5 eindeutige Status)
    """
    zone_name, icon_id, status_text, mqtt_status = get_moisture_zone(adc_value)
    return mqtt_status

def get_icon_for_adc(adc_value):
    """
    Gibt Icon und Text für ADC-Wert zurück
    Returns: tuple (icon, status_text)
    """
    zone_name, icon_id, status_text, mqtt_status = get_moisture_zone(adc_value)
...
def print_moisture_zones():
    ...
    print("="*70)
    print(f"{'Zone':<15} {'ADC-Bereich':<15} {'Icon':<10} {'Text':<12} {'MQTT':<6}")
    print("-"*70)
    for name, adc_min, adc_max, icon_id, text, mqtt in MOISTURE_ZONES:
        adc_range = f"{adc_min}-{adc_max}"
        icon_name = f"Icon #{icon_id}"
        print(f"{name:<15} {adc_range:<15} {icon_name:<10} {text:<12} {mqtt:<6}")
    print("="*70)
...
def show_icon_screen(display, adc_value):
    """Zeigt Icon-Screen basierend auf ADC-Wert (Position: x=16, y=0)"""
    display.fill(0)
    icon, text = get_icon_for_adc(adc_value)
    draw_large_icon(display, icon, 16, 0)
    display.text(text, 10, 56)
    display.show()

def show_icon_screen_debug(display, adc_value, force_icon=None):
    """Debug-Funktion für Icon-Tests"""
    display.fill(0)

    if force_icon:
        icons = {1: ICON_WAVES, 2: ICON_FLOWER_1L, 3: ICON_FLOWER_2B,
                4: ICON_FLOWER_1R, 5: ICON_WATERING}
        texts = {1: "  FEUCHT", 2: " WACHSEND", 3: " OPTIMAL",
                4: " WELKEND", 5: " GIESSEN!"}
        icon = icons[force_icon]
        text = texts[force_icon]
    else:
        icon, text = get_icon_for_adc(adc_value)

    draw_large_icon(display, icon, 16, 0)
    display.text(text, 10, 56)
    display.text(f"ADC:{adc_value}", 0, 44)
    display.show()
...
def display_values(vbat, vsensor, status, count):
    ...
    try:
        display.fill(0)
        display.text("Bat:%.3fV" % vbat, 0, 0)
        display.text("Sen:%.3fV" % vsensor, 0, 10)
...
def main():
    ...
    while True:
        ...
        # Screen 3: Icon-Screen mit automatischer Zone-Erkennung (40×40 @ x=16,y=0)
